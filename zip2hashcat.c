/*
 * zip2hashcat processes input ZIP files into a format suitable for use with hashcat.
 * zip2hashcat is based on zip2john.
 *
 * This software is
 ...
 * Copyright (c) 2023 David Bolvansky,
 ...
 * 2. Run hashcat on the output generated by zip2hashcat.
 *
 * Output Line Format:
 *
 ...
 *
 * The newer $pkzip2$ addressed the problem by adding TC (as in timestamp), but still neither zip2hashcat or
 ...
 *    Ma = magic (file magic).  This is reservered for now.  See pkzip_fmt_plug.c or zip2hashcat.c for information.
 ...
 *        was seen when running zip2hashcat. If the file is removed, this hash line will no longer be valid.
...
uint64_t fget64LE(FILE *fp)
{
	uint64_t v = (uint64_t)fgetc(fp);
	v |= (uint64_t)fgetc(fp) << 8;
	v |= (uint64_t)fgetc(fp) << 16;
	v |= (uint64_t)fgetc(fp) << 24;
	v |= (uint64_t)fgetc(fp) << 32;
	v |= (uint64_t)fgetc(fp) << 40;
	v |= (uint64_t)fgetc(fp) << 48;
	v |= (uint64_t)fgetc(fp) << 56;
	return v;
}

uint32_t fget32LE(FILE *fp)
{
